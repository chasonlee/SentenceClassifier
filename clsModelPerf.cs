...
namespace NeuroNetworkClassifier
{
	...
	public class clsModelPerf
	{
		...
		public void savePerfLog(string fileName, string msg, double perf, bool enter)
		{
			using (FileStream fs = new FileStream(fileName, FileMode.Append))
			{
				StreamWriter sw = new StreamWriter(fs);
				sw.WriteLine(DateTime.Now + " " + msg + perf);
				if (enter == true) sw.WriteLine();
				sw.Close();
			}
			System.Console.WriteLine("性能日志已保存！");
		}
		...
		/// <returns></returns>
		public double trainSetPerf(clsSentVector sentvec, clsNNTrainer trainer)
		{
			double perf = 0;
			...
			for (int i = 0; i < sentvec.trainSetVec.Count(); i++)
			{
				...
				//比较输出与标签，并记录出错的下标
				if (sameOutput(foutput, sentvec.trainSetLabel[i])) perf++;
				else 
				{
			...
			perf = perf / sentvec.trainSetVec.Count();
			return perf;
		}
		...
		/// <param name="trainer"></param>
		/// <returns></returns>
		public double testSetPerf(clsSentVector sentvec, clsNNTrainer trainer)
		{
			double perf = 0;
			double[] output = new double[sentvec.typeNum];
			testErrInx.Clear();
			for (int i = 0; i < sentvec.testSetVec.Count(); i++)
			{
				...
				double[] foutput = formatOutput(output);
				//比较输出与标签，并记录出错的下标
				if (sameOutput(foutput, sentvec.testSetLabel[i])) perf++;
				else
				{
			...
			perf = perf / sentvec.testSetVec.Count();
			return perf;
		}
		/// <summary>
		/// 保存出错的例子
