...
namespace NeuroNetworkClassifier
{
    ...
    public class DrawingCurve
    {
        ...
        /// <returns></returns>
		public Bitmap DrawImage(Curve2D cuv2D)
        {
    ...
    public class Curve2D
    {
        ...
        private float fltWidth = 430; //图像宽度
        private float fltHeight = 510; //图像高度
		...
		private string strTitle = "模型性能曲线图"; //标题
        private string strXAxisText = "迭代次数"; //X轴说明文字
        private string strYAxisText = "误差率%"; //Y轴说明文字
        ...
        public void Fit()
        {
            ...
            int intYSliceCount = (int)((fltMaxValue + 10.0) / YSliceValue);
        ...
        {
            float fltX = Width - XSpace + XSlice / 2 - (XAxisText.Length - 1) * intFontSpace + 50;
        ...
        private void SetXAxis(ref Graphics objGraphics)
        {
			...
			int span = Keys.Length / 5;
			int span_dash = Keys.Length / 20;
			if (span == 0) span = 1;
			if (span_dash == 0) span_dash = 1;
            ...
            for (int i = 0; i <= iWidth; i += 10) //以10为单位
            {
                ...
                if (iCount == 5)
                {
                    ...
                    //这里显示X轴刻度
					if (iSliceCount <= Keys.Length - 1 && (iSliceCount % span == 0 && iSliceCount < Keys.Length - span || iSliceCount == Keys.Length - 1))
                    {
